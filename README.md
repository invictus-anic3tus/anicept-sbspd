<div align="center">

  <h1>The Anicept SBSPD</h1>

  <img width="848" height="773" alt="title image" src="https://github.com/user-attachments/assets/d7168fde-21c0-4958-9590-42c21a06543c" />

  <h3>By Anicetus</h3>

  <p display="inline">

  <a href="https://creativecommons.org/licenses/by-nc/4.0/">
    <img src="https://licensebuttons.net/l/by-nc/4.0/88x31.png" alt="Creative Commons Attribution-NonCommercial 4.0 International License"></a> <a href="https://hackclub.com/">
    <img alt="Funded by Hack Club" src="https://img.shields.io/badge/Hack_Club-Funded-ec3750?style=for-the-badge&logo=hackclub&logoColor=ec3750"></img>
  </a>

  </p>

  <h4>The Self Built Solder Paste Dispenser! Able to be mounted onto just about any 3D printer, this device adds a holder for a solder paste syringe, fully replacing the need to buy a solder stencil.</h4>
</div>

<sub>This work is licensed under a
[Creative Commons Attribution-NonCommercial 4.0 International License](https://creativecommons.org/licenses/by-nc/4.0/).
</sub>

## About

Hi! I'm Anicetus, and I love 3D printers and making custom hardware. Some of my favorite hardware to make is custom PCBs, namely the [Anicept Tempus](https://github.com/invictus-anic3tus/tempus), which often use very tiny SMD parts. The common choice is to go buy a solder stencil, which allows solder paste to be spread onto the PCB and easily reflowed. But stencils can get _expensive_. And, of course, they can only be used for one specific PCB! If your project needs a redesign, that means you need a whole new stencil for it.

So I got to thinking. Way back when, all plastic parts were made with injection molding, which had the same problems as solder stencils do now: to make a different plastic part, you need a whole new mold, which is very expensive. Then they invented 3D printers. These devices move in all three dimensions to make almost _any_ plastic part you want--why shouldn't the stencil problem have the same solution?

Well, maybe there is a good reason that this isn't so widespread. In any case, I'm about to find out.

## Features

- A Nema 11 stepper motor for accurate and high-power control over the solder syringe
- An easy-to-use clamp/hinge system that holds everything tightly in place
- A magnet-based mounting system that's compatable with any 3D printer
- Only four different screws/nuts!
- A fully 3D printable case
- A compact form factor
- ... all this for $15!

## Assembly + Usage

Assembly video coming soon! For now, please reference the CAD design to see how everything fits together :)

## Bill of Materials

| Part         | Link                                                                    | Price  | Notes                                                     |
|--------------|-------------------------------------------------------------------------|--------|-----------------------------------------------------------|
| Nema 11      | https://www.aliexpress.us/item/2255799884723806.html                    | $5.58  | 11HS3410                                                  |
| Motor Driver | https://www.aliexpress.us/item/3256805074646808.html                    | $0.99  | TMC2209                                                   |
| M5x50 SHCS   | https://www.aliexpress.us/item/2255800598515019.html                    | $0.99  | For plunger + hinge                                       |
| M2.5x16 SHCS | https://www.aliexpress.us/item/2255800598515019.html                    | $0.99  | For motor + clamp                                         |
| M5 Nut       | https://www.aliexpress.us/item/2255800416538696.html                    | $0.99  | For hinge + gear                                          |
| M2.5 Nut     | https://www.aliexpress.us/item/2255800416538696.html                    | $0.99  | For clamp                                                 |
| Magnets      | https://www.aliexpress.us/item/3256809809310037.html                    | $0.99  | 4x2mm - required over ten for the PCB-bed mount standoffs |
| Adhesive     | https://www.aliexpress.us/item/3256809785876209.html                    | $0.99  | For attaching the mount to the 3D printer                 |
| Solder Paste | https://www.amazon.com/SMD291AX-REWORK-SOLDER-PASTE-CLEAN/dp/B0186IM0P0 | $14.99 | Any solder paste in Chip Quik's 5CC syringe               |
| Sum          |                                                                         | $27.50 |                                                           |
| Grand Total  |                                                                         | $29.77 |                                                           |
| Paste Owned: |                                                                         | $13.54 |                                                           |

## Contributing

<sub>aha somebody wishes to help me i see</sub>

If you'd like to request changes, suggest additions, or forcefully make me edit things, feel free to contact me via email (`me at anicetus dot dev`, preferred) or Discord (anic3tus).
